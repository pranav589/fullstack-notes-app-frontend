...
export default function EditNote({match}){
  ...
  useEffect(()=>{
    const getNote=async ()=>{
      ...
      if(match.params.id){
        const res=await axios.get(`https://fullstack-notes-app.glitch.me/notes/${match.params.id}`,{
  ...
  const editNote=async(e)=>{
    ...
    try{
      ...
      if(token){
        ...
        await axios.put(`https://fullstack-notes-app.glitch.me/notes/${id}`,newNote,{
          headers:{Authorization:token}
        })
