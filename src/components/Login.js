...
export default function Login({setIsLogin}){
  ...
  const registerSubmit=async e=>{
    e.preventDefault()
    try{
      const res=await axios.post('https://fullstack-notes-app.glitch.me/users/register',{
        username:user.name,
        email:user.email,
  ...
  const loginSubmit=async e=>{
    e.preventDefault()
    try{
      const res=await axios.post("https://fullstack-notes-app.glitch.me/users/login",{
        email:user.email,
        password:user.password
      })
